...
function edit_delivery_person(id) {
    $.ajax({
        ...
        success: function (result) {
            if (result.success == true) {
                $("#update_zone_area_form").attr(
                    "action",
                    base_url + "/admin/delivery_zone_area/" + result.data.id
                );
                ...
                if (result.data.vendor_id.length > 0) {
                    ...
                    vendor.forEach((element) => {
                        $(
                            'select[id="edit_vendor_id"] option[value=' +
                                element +
                                "]"
                        ).attr("selected", true);
                        $(
                ...
                const edit_map = new google.maps.Map(
                    document.getElementById("edit_map"),
                    {
                ...
                const edit_autocomplete = new google.maps.places.Autocomplete(
                    edit_input
                );
                ...
                edit_autocomplete.addListener("place_changed", () => {
                    ...
                    $("#edit_lat").val(
                        edit_place.geometry.location.lat().toFixed(5)
                    );
                    $("#edit_lang").val(
                        edit_place.geometry.location.lng().toFixed(5)
                    );

                    var edit_latlng = new google.maps.LatLng(
                        edit_place.geometry.location.lat().toFixed(5),
                        edit_place.geometry.location.lng().toFixed(5)
                    );
...
function delivery_zone_area_map(id) {
    $.ajax({
        ...
        type: "GET",
        url:
            base_url + "/admin/delivery_zone_area/delivery_zone_area_map/" + id,
        success: function (result) {
            if (result.success == true) {
                ...
                const show_map = new google.maps.Map(
