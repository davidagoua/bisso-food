...
$(document).ready(function () {
    ...
    //     $(this).val(val);
    // });​
    $(document).on("keyup", ".floatTextBox", function (event) {
        var val = $(this).val();
        if (isNaN(val)) {
            val = val.replace(/[^0-9\.]/g, "");
            if (val.split(".").length > 1) val = val.replace(/\.+$/, "");
        }
        $(this).val(val);
    });
    ...
    if (
        window.location.origin + window.location.pathname ==
        $("#mainurl").val() + "/vendor/vendor_home"
    ) {
        $.ajax({
            headers: {
                "X-CSRF-TOKEN": $('meta[name="csrf-token"]').attr("content"),
            ...
            reader.onload = function (e) {
                ...
                if (extFile == "jpg" || extFile == "jpeg" || extFile == "png") {
                    ...
                    if (type == "add") {
                        $("#image").attr(
                            "src",
                            base_url + "/images/upload/impageplaceholder.png"
                        );
                    }
                }
            };
        ...
        if (input.files && input.files[0]) {
            ...
            reader.onload = function (e) {
                ...
                var extFile = fileName
                    ...
                    .toLowerCase();
                if (extFile == "jpg" || extFile == "jpeg" || extFile == "png") {
                    $(".logoContainer #national_identity").attr(
                        "src",
                        e.target.result
                    );
                } else {
    ...
    function readLicnDoc(input) {
        if (input.files && input.files[0]) {
            ...
            reader.onload = function (e) {
                var fileName =
                    document.getElementById("previewlicence_doc").value;
                var idxDot = fileName.lastIndexOf(".") + 1;
                var extFile = fileName
                    .substr(idxDot, fileName.length)
                    .toLowerCase();
                if (extFile == "jpg" || extFile == "jpeg" || extFile == "png") {
                    $(".logoContainer #licence_doc").attr(
                        "src",
                        e.target.result
                    );
                } else {
    ...
    $("input[type=checkbox][name=verification_email]").change(function () {
        $(".setting_alert").hide();
        var user_verification = $("input[type=checkbox][name=verification]").is(
            ":checked"
        );
        var mail_verification = $(
    ...
    $("input[type=checkbox][name=is_driver_accept_multipleorder]").change(
        function () {
            if (this.checked == true) {
...
function avarageAdminItems(data) {
    ...
    var site_color = getComputedStyle(
        document.documentElement
    ).getPropertyValue("--site_color");
    var hover_color = getComputedStyle(
        document.documentElement
...
function avarageVendorItems(data) {
    if (document.getElementById("vendorAvarageTime")) {
        ...
        var myChart = new Chart(ctx, {
            ...
            data: {
                datasets: [
                    {
                        data: [data.currentMonth, data.lastMonth],
                        backgroundColor: ["#55D8FE", "#FF8373"],
                        label: "Dataset 1",
                    },
                ],
                labels: [
...
function update_custimization(i, name) {
    ...
    $("#custom" + i).append(
        '<tr id="custom' +
            ...
            name +
            '[]" class="form-control" required placeholder="item" style="text-transform: none;"></td><td><input type="text" name="price[]" class="form-control floatTextBox" required placeholder="price"></td><td><input type="radio" name="isdefault_' +
            name +
            '" value="' +
            tds +
            '" id="' +
            ...
            "_" +
            tds +
            '">&nbsp;Default</label></td><td><input type="checkbox" id="status' +
            i +
            "_" +
            tds +
            '" name="status' +
            ...
            "_" +
            tds +
            '">Status</label></td><td><button type="button" class="btn btn-primary removebtn"><i class="fas fa-times"></i></button></td></tr>'
    );
}

function add_field() {
    $(".delivery_table").append(
        '<tr><td><input type="number" required name="min_value[]" class="form-control"></td><td><input type="number" required name="max_value[]" class="form-control"></td><td><input type="text" required name="charges[]" class="form-control floatTextBox"></td><td><button type="button" class="btn btn-primary removebtn"><i class="fas fa-times"></i></button></td></tr>'
    );
}

function add_cancel_reason() {
    $(".cancel_reason").append(
        '<tr><td><input type="text" name="cancel_reason[]" class="form-control" required></td><td><button type="button" class="btn btn-primary removebtn"><i class="fas fa-times"></i></button></td></tr>'
    );
}

function add_drivervehical_field() {
    $(".driver_vehical_table").append(
        '<tr><td><input type="text" name="vehical_type[]" class="form-control" required></td><td><select name="license[]" class="form-control"><option value="yes">Yes</option><option value="no">no</option></select></td><td><button type="button" class="btn btn-primary removebtn"><i class="fas fa-times"></i></button></td></tr>'
    );
}

function add_driverearning_field() {
    $(".driver_earning_table").append(
        '<tr><td><input type="number" name="min_km[]" class="form-control" required></td><td><input type="number" name="max_km[]" class="form-control" required></td><td><input type="text" name="charge[]" class="form-control floatTextBox" required></td><td><button type="button" class="btn btn-primary removebtn"><i class="fas fa-times"></i></button></td></tr>'
    );
}
...
function addhours(i) {
    ...
    var timeslot = $("#timeslot").val();
    $("#tr" + i).after(
        '<tr><td><input class="timeslots" name="start_time_' +
            i +
            '[]" /></td><td><input class="timeslots" name="end_time_' +
            i +
            '[]"/></td><td><button type="button" class="removebtn btn btn-danger text-light"><i class="fas fa-times"></i></button></tr>'
    );
    $(".timeslots").timepicker({
        timeFormat: "h:mm p",
        defaultTime: "11",
    ...
    $.ajax({
        ...
        success: function (result) {
            if (result.success == true) {
                ...
                $(".show_admin_commission").text(
                    result.data.currency + result.data.order.admin_commission
                );
                $(".show_vendor_amount").text(
                    result.data.currency + result.data.order.vendor_amount
                );
                $(".TaxTable").html("");
                ...
                result.data.order.orderItems.forEach((element) => {
                    $(".show_order_table").append(
                        "<tr><td>" +
                            ...
                            '</td><td class="show_custimization_name' +
                            element.id +
                            '"></td><td class="show_custimization_price' +
                            element.id +
                            '"></td></tr>'
                    );
                    if (element.custimization != null) {
                        element.custimization.forEach((custimization) => {
                            $(".show_custimization_name" + element.id).text(
                                custimization.data.name
                            );
                            $(".show_custimization_price" + element.id).text(
                                result.data.currency + custimization.data.price
                            );
                        });
                    } else {
                        $(".show_custimization_name" + element.id).text(
                            "doesn't apply any custimization"
                        );
                    }
                });
                $(".show_delivery_at").text(result.data.order.delivery_type);
                $(".show_total_amount").text(
                    result.data.currency + result.data.order.amount
                );
                if (result.data.order.promocode_price != null) {
                    $(".show_discount").text(
                        result.data.currency + result.data.order.promocode_price
                    );
                } else if (result.data.order.vendor_discount_price != null) {
                    $(".show_discount").text(
                        result.data.currency +
                            result.data.order.vendor_discount_price
                    );
                } else {
    ...
    $.ajax({
        ...
        success: function (result) {
            if (result.success == true) {
                $(".details_body").html("");
                $(".details_body").append(
                    '<div class="table-responsive"><table class="table"><thead class="display"><tr><th>Date</th><th>Vendor earning</th><th>Settlement with</th><th>Payment token</th></tr></thead></table></div>'
                );
                if (result.data.length != 0) {
                    result.data.forEach((element) => {
                        if (element.vendor_status == 1) {
                            var token =
                                element.payment_token == null
                                    ? "Payment with cod"
...
function show_driver_settle_details(index, id) {
    if (typeof check === "function") {
        ...
        $.ajax({
            ...
            url:
                base_url +
                "/admin/show_driver_settle_details/" +
                duration +
                "/" +
                id,
            success: function (result) {
                    ...
                    if (result.data.length != 0) {
                        result.data.forEach((element) => {
                            if (element.driver_status == 1) {
                                var token =
                                    element.driver_payment_token == null
                                        ? "Payment with cod"
                                        : element.driver_payment_token;
                                $(".display").after(
                                    "<tbody><td>" +
                                        element.date +
                                        "</td><td>" +
                                        result.currency +
                                        element.driver_earning +
                                        "</td><td>" +
                                        element.driver_payment_type +
                                        "</td><td>" +
                                        token +
                                        "</td></tbody>"
                                );
            ...
            success: function (result) {
                if (result.success == true) {
                    $("#notification_content").val(
                        result.data.notification_content
                    );
                    ...
                    $(".edit_notification_template_form").attr(
                        "action",
                        base_url +
...
function update_submenu(id) {
    if (typeof check === "function") {
        ...
        $.ajax({
            ...
            success: function (result) {
                $("#update_submenu_form").attr(
                    "action",
                    base_url + "/admin/submenu/" + result.data.id
                );
                ...
                if (result.data.qty_reset == "never") {
                    ...
                    $("input[name=item_reset_value]").prop("disabled", false);
                    $("input[name=item_reset_value]").val(
                        result.data.item_reset_value
                    );
        ...
        success: function (result) {
            if (result.success == true) {
                ...
                if (result.data.status == "PENDING") {
                    ...
                    $(".orderStatusTd" + id).html(
                        '<span class="badge badge-pill pending order_status' +
                ...
                if (result.data.status == "APPROVE") {
                    $(".orderStatusTd" + id).html("");
                    $(".orderStatusTd" + id).html(
                        '<span class="badge badge-pill approve order_status' +
                            result.data.order_id +
                            '">APPROVE</span>'
                    );
                }
                ...
                if (result.data.status == "REJECT") {
                    $(".orderStatusTd" + id).html("");
                    $(".orderStatusTd" + id).html(
                        '<span class="badge badge-pill reject order_status' +
                            result.data.order_id +
                            '">REJECT</span>'
                    );
                }
                ...
                if (result.data.status == "CANCEL") {
                    $(".orderStatusTd" + id).html("");
                    $(".orderStatusTd" + id).html(
                        '<span class="badge badge-pill cancel order_status' +
                            result.data.order_id +
                            '">CANCEL</span>'
                    );
                }
                ...
                if (result.data.status == "PICKUP") {
                    $(".orderStatusTd" + id).html("");
                    $(".orderStatusTd" + id).html(
                        '<span class="badge badge-pill pickup order_status' +
                            result.data.order_id +
                            '">PICKUP</span>'
                    );
                }
                ...
                if (result.data.status == "DELIVERED") {
                    $(".orderStatusTd" + id).html("");
                    $(".orderStatusTd" + id).html(
                        '<span class="badge badge-pill delivered order_status' +
                            result.data.order_id +
                            '">DELIVERED</span>'
                    );
                }
                ...
                if (result.data.status == "COMPLETE") {
                    $(".orderStatusTd" + id).html("");
                    $(".orderStatusTd" + id).html(
                        '<span class="badge badge-pill complete order_status' +
                            result.data.order_id +
                            '">COMPLETE</span>'
                    );
                }
                ...
                if (result.data.status == "PREPARE_FOR_ORDER") {
                    $(".orderStatusTd" + id).html("");
                    $(".orderStatusTd" + id).html(
                        '<span class="badge badge-pill preparre-food order_status' +
                            result.data.order_id +
                            '">PREPARE FOR ORDER</span>'
                    );
                }
                ...
                if (result.data.status == "READY_FOR_ORDER") {
                    $(".orderStatusTd" + id).html("");
                    $(".orderStatusTd" + id).html(
                        '<span class="badge badge-pill ready_for_food order_status' +
                            result.data.order_id +
    ...
    if (typeof check === "function") {
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                $("#update_cutimization_form").attr(
                    "action",
                    base_url + "/admin/customization_type/" + result.data.id
                );
                $("#update_name").val(result.data.name);
                $("#update_min_item_selection").val(
                    result.data.min_item_selection
                );
                $("#update_max_item_selection").val(
                    result.data.max_item_selection
                );
...
function update_menu(id) {
    if (typeof check === "function") {
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result.success == true) {
                    $("#update_menu_form").attr(
                        "action",
                        base_url + "/admin/menu/" + result.data.id
                    );
                    $("#update_image").attr("src", result.data.image);
    ...
    if (typeof check === "function") {
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                if (result.success == true) {
                    $("#update_image").attr("src", result.data.image);
                    $("#update_cuisine_form").attr(
                        "action",
                        base_url + "/admin/cuisine/" + result.data.id
                    );
    ...
    if (typeof check === "function") {
        ...
        $.ajax({
            ...
            success: function (result) {
                console.log("result", result);
                if (result.success == true) {
                    $("#update_banner_form").attr(
                        "action",
                        base_url + "/admin/banner/" + result.data.id
                    );
                    $("#update_image").attr("src", result.data.image);
                    $("#update_name").val(result.data.name);
                }
