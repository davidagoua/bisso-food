...
@section('content')
    ...
    <section class="section">
        ...
        <div class="section-body">
            ...
            <div class="card">
                <div class="card-body">
                    <form class="container-fuild myform" action="{{ url('admin/user') }}" method="post">
                        @csrf
                        <div class="row">
                            <div class="col-md-12 mb-3">
                                <label for="name">{{ __('user name') }}</label>
                                <input type="text" name="name"
                                    class="form-control @error('title') is_invalide @enderror" id=""
                                    placeholder="{{ __('user name') }}" value="{{ old('name') }}" required=""
                        ...
                        <div class="row">
                            <div class="col-md-12 mb-3">
                                ...
                                <input type="email" name="email_id"
                                    class="form-control @error('email_id') is_invalide @enderror"P
                        ...
                        <div class="row">
                            <div class="col-md-12 mb-3">
                                ...
                                <input type="password" name="password"
                                    ...
                                    placeholder="{{ __('* * * * * *') }}" value="{{ old('password') }}" required=""
                                    style="text-transform: none;">
                                @error('password')
                                    <span class="custom_error" role="alert">
                                        <strong>{{ $message }}</strong>
                                ...
                                <select class="form-control select2" name="roles[]" id="">
                                    @foreach ($roles as $role)
                                        <option value="{{ $role->id }}"
                                            {{ $role->title == 'vendor' ? 'disabled' : '' }}>{{ $role->title }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
